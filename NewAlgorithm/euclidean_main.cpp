...
int main()
{
	...
	cout<<"Distance: "<<calcEuclidean(one, two)<<endl;
...
int main()
{
	...
	for(int i = 0; i < ratingMatrix.size(); i++)
	{
		...
		// This will hold the Euclidean distances for the current user
		...
		for(int j = 0; j < NUM_FEATURES; j++)
		{
			...
			distance[j + 1] = euclidean(vector<double>(ratingMatrix[i].begin() + 1, ratingMatrix[i].end()), vector<double>(ratingMatrix[userMap[userVector[j]]].begin() + 1, ratingMatrix[userMap[userVector[j]]].end()));
		...
		// Add the distance to the Euclidean Distance Matrix
		euclideanDistances.push_back(distance);
	}
	
	cout<<"Dimensions of the Euclidean Distance Matrix: "<<euclideanDistances.size()<<", "<<euclideanDistances[0].size()<<endl;
	...
	{
		for(int j = 1; j < euclideanDistances[i].size(); j++)
			outfile<<i + 1<<" "<<j<<" "<<euclideanDistances[i][j]<<endl;
	}
